 ...
 * Contest: 1761/C | Link: https://codeforces.com/contest/1761/problem/C
 *
 * ── Problem ──────────────────────────────────────────────
 * You are given a binary matrix b (all elements of the matrix are 0 or 1) of n rows and n columns.
 * You need to construct a n sets A_1, A_2, ..., A_n, for which the following conditions are satisfied:
 * 
 * • Each set is nonempty and consists of distinct integers between 1 and n inclusive.
 * • All sets are distinct.
 * • For all pairs (i,j) satisfying 1≤q i, j≤q n, b_(i,j)=1 if and only if A_i A_j. In other words, b_(i, j) is 1 if A_i is a proper subset of A_j and 0 otherwise. Set X is a proper subset of set Y, if X is a nonempty subset of Y, and X ≠ Y.
 ...
 * For the i-th line, first output s_i (1 ≤ s_i ≤ n) — the size of the set A_i. Then, output s_i distinct integers from 1 to n — the elements of the set A_i.
 * If there are multiple solutions, you can output any of them.
 * It's guaranteed that for all test cases in this problem, such n sets exist.
 *
 * ── Example Input ────────────────────────────────────────
