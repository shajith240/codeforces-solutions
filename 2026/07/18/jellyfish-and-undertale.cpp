 ...
 * Contest: 1875/A | Link: https://codeforces.com/contest/1875/problem/A
 *
 * ── Problem ──────────────────────────────────────────────
 ...
 * 
 * • You will choose some (possibly none) of your tools that have not been used before. If you choose the i-th tool, and the bomb's timer is currently set to c, the timer will be changed to (c + x_i, a).
 * • The timer decreases by 1.
 * • If the timer reaches 0, the bomb explodes. Jellyfish now wants to know the maximum time in seconds until the bomb explodes if the tools are used optimally.
 *
 * ── Input ────────────────────────────────────────────────
 * Each test contains multiple test cases. The first line contains the number of test cases t (1 ≤q t ≤q 2000). The description of the test cases follows.
 * The first line of each test case contains three integers a, b and n (1 ≤q b ≤q a ≤q 10^9, 1 ≤q n ≤q 100) — the maximum value of the bomb's timer, the initial value of the timer of the bomb and the number of tools.
 * The second line of each test contains n integers x_1, x_2, ..., x_n (1 ≤q x_i ≤q 10^9) — the number the timer can increase by using the i-th tool.
 ...
 * • Second 9: the timer decreases by 1, then c=0. The bomb explodes. It can be proved that there is no way to use the tools such that the bomb explodes after more than 9 seconds.
 *
 */
...
void solve(){
   int a; //the max time the timer can go
   int b;// init timer 
   int n;//number of tools we have
