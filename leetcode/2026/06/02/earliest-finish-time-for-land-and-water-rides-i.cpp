    ...
    int earliestFinishTime(vector<int>& landStartTime, vector<int>& landDuration, vector<int>& waterStartTime, vector<int>& waterDuration) {
        ...
        for(int i = 0 ; i < n ; i++){
            for(int j = 0 ; j < m  ; j++){
               ...
               int land_ride_first = max(landStartTime[i] + landDuration[i],waterStartTime[j]) + waterDuration[j];
               
               int water_ride_first = max(waterStartTime[j] + waterDuration[j],landStartTime[i]) + landDuration[i];
        
                ans = min(ans,min(land_ride_first,water_ride_first));
            }
        }
